  ...
  for (int i = 0; i < total; i++)
  {
    if (strcmp(argv[1], commands[i].name) == 0)
    {
      if (commands[i].requires_package == 1)
      {
        ...
        {
          if (j > 2) strncat(packages, " ", sizeof(packages) - strlen(packages) - 1);
          strncat(packages, argv[j], sizeof(packages) - strlen(packages) - 1);
        }

        snprintf(command, sizeof(command), commands[i].pacman, packages);
      }
      else
      {
